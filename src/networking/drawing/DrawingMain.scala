...
import java.util.regex.Pattern.NotBehind
...
object DrawingMain extends JFXApp {
  ...
  private var server: Option[RemoteCollaborationServer] = None
  private var client: Option[Collaborator] = None
    ...
    scene = new Scene(800, 600) {
      ...
      openItem.onAction = (ae: ActionEvent) => {
        ...
        if (file != null) {
          ...
          if (drawing != null) {
            addDrawing(drawing, file.getName)
          }
        }
      }
      ...
      joinItem.onAction = (ae: ActionEvent) => {
        if (server.isEmpty) {
          val machine = showInputDialog("What machine is the server on?", "Server Host")
          machine.foreach { m =>
            try {
              Naming.lookup("rmi://"+m+"/CollabServer") match {
                case rcs: RemoteCollaborationServer =>
                  server = Some(rcs)
                  client = Some(new Collaborator(rcs))
                case _ => throw new NotBoundException("Server type not found.")
              }
            } catch {
              case (_: NotBoundException) | (_: RemoteException) =>
                val s = new CollaborationServer
                Naming.rebind("CollabServer", s)
                server = Some(s)
                client = Some(new Collaborator(s))
            }
          }
        }
        client.foreach { _.stage.show }
      }
      ...
      while (true) {
        ...
        ois.readObject() match {
          case drawing: Drawing =>
            Platform.runLater { addDrawing(drawing, title) }
          case _ =>
            println("Non-drawing received.")
        }
        ois.close()
        sock.close()
      }
  ...
  private def addDrawing(d: Drawing, name: String): Unit = {
    val (tab, tree) = makeDrawingTab(d, "Untitled")
  ...
  private def sendDrawing(): Unit = {
    ...
    if (current >= 0) {
      val (drawing, _) = drawings(current)
      val host = showInputDialog("What machine do you want to send the drawing to?", "Machine Name")
      host.foreach { machine =>
        val sock = new Socket(machine, 8088)
        val oos = new ObjectOutputStream(new BufferedOutputStream(sock.getOutputStream))
        val title = showInputDialog("What is the title of the drawing?", "Title")
        title.foreach { t =>
          oos.writeUTF(t)
          oos.writeObject(drawing)
          oos.close()
  ...
  private def showInputDialog(message: String, title: String): Option[String] = {
